...
let rawApiKey = (useFileConfig ? fileConfig.apiKey : (import.meta.env.VITE_FIREBASE_API_KEY || "")).trim();
if (rawApiKey.startsWith('"') && rawApiKey.endsWith('"')) {
  rawApiKey = rawApiKey.substring(1, rawApiKey.length - 1);
}
if (rawApiKey.startsWith("'") && rawApiKey.endsWith("'")) {
  rawApiKey = rawApiKey.substring(1, rawApiKey.length - 1);
}

const firebaseConfig = useFileConfig ? { ...fileConfig, apiKey: rawApiKey } : {
  apiKey: rawApiKey,
